...
def process_frame_with_pose_estimation(image, display=False):
    """
    ...
    if results.pose_landmarks:
        ...
        if display:
            # Draw detected pose landmarks (joints) on image
...
def detect_splash(frame_before_entry, entry_frame, display=False):
    """
    ...
    display: If True, displays the splash detection result
    ...
    if contours:
        ...
        splash_size = (w, h)
        if display:
            cv2.rectangle(entry_frame, (x, y), (x + w, y + h), (0, 255, 0), 2)

    # Display the result if display is True
    if display:
        cv2.imshow("Splash Detection", entry_frame)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
def get_difficulty(dive_type, position):
    ...
    normalized_dive_types = {key.lower(): {pos.lower(): value for pos, value in positions.items()} for key, positions in difficulty_dict.items()}
...
def process_videos_in_folder(input_folder, output_folder, json_file, output_json_path):
    """
    Processes all video files in the input folder and runs the annotation pipeline for each video
    
    Parameters:
    input_folder: Path to input folder with video files
    output_folder: Path to output folder
    json_file: Path to the JSON file with dive labels
    output_json_path: Path to save the final JSON file with all labeled results
    """
    ...
    for video_file in video_files:
        ...
        os.makedirs(output_folder, exist_ok=True)
        # Get the dive type, position, and score
        dive_type, position, score = get_dive_info_from_json(json_file, os.path.basename(video_file))

        # Run the automated annotation process for the video
        print(f"Processing video: {video_file} with dive type: {dive_type}, position: {position}, score: {score}")
        result = automate_dive_annotation(video_file, output_folder, dive_type, position, score)
...
if __name__ == "__main__":
    process_videos_in_folder("Shortened videos/videos", "Shortened videos/Frames and labeling/Extracted frames", json_file="Shortened videos/olympic_dive_types.json", output_json_path="Shortened videos/Frames and labeling/labeled_results.json")